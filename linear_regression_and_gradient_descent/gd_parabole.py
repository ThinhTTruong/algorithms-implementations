# Solving a parabolic-shape dataset using linear regression, 
# and applying gradient descent method to compare results with linear regression method 
...
# Check if the gradient of x is correct using approximation
def check_grad(x):
	eps = 1e-4
	g = np.zeros_like(x)	
...
#  Apply gradient descent to the initial value with the given learning rate and iteration
...
x_formula = np.linalg.inv(A.transpose().dot(A)).dot(A.transpose()).dot(b)
x0_gd = np.linspace(1,46,10000)
y0_fomular = x_formula[0][0] + x_formula[1][0] * x0_gd + x_formula[2][0] * x0_gd * x0_gd
